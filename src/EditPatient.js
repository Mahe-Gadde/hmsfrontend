...
const EditPatient = ({ patientId, onClose, onUpdate }) => {
  ...
  useEffect(() => {
    const fetchPatient = async () => {
      try {
        const response = await axios.get(`http://hospitalmanagement-env.eba-p9txdymv.eu-north-1.elasticbeanstalk.com/patients/${patientId}`);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios.put(`http://hospitalmanagement-env.eba-p9txdymv.eu-north-1.elasticbeanstalk.com/patients/${patientId}`, patientData);
      onUpdate();
    } catch (error) {
